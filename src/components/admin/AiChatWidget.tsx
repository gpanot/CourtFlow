...
const CACHED_EN: Record<string, string> = {
  ...
  bookings: "To manage court bookings:\n1. Go to **Bookings** in the left menu (CourtPass - Booking section)\n2. Select the venue — the Day Planner grid shows courts as columns and time slots as rows\n3. The grid defaults to **1h view**; toggle to **30-min view** for finer granularity\n4. Click an available slot to create a booking (players can book multi-court group bookings)\n5. Click an existing booking to edit or cancel it\n6. Use the date arrows to navigate days\n7. Blocked slots appear as colored blocks: Open Play (green), Competition (blue), Private Event (amber), Private Competition (orange), Maintenance (grey), Alobo (violet)\n8. Go to **Schedule** tab to configure recurring weekly sessions\n9. Go to **Pricing** tab to set per-day/hour pricing rules\n\nCoaching lessons also appear on the same booking grid — manage them from the **Coaching** page",
...
const CACHED_VI: Record<string, string> = {
  ...
  bookings: "Để quản lý đặt sân:\n1. Vào **Bookings** ở menu bên trái (phần CourtPass - Booking)\n2. Chọn venue — lưới Day Planner hiển thị sân theo cột và khung giờ theo hàng\n3. Lưới mặc định hiển thị theo **1 giờ**; chuyển sang **30 phút** để xem chi tiết hơn\n4. Nhấn ô trống để tạo đặt sân (người chơi có thể đặt nhóm nhiều sân)\n5. Nhấn vào đặt sân hiện có để sửa hoặc hủy\n6. Dùng mũi tên ngày để điều hướng\n7. Ô bị khóa hiển thị dạng khối màu: Open Play (xanh lá), Competition (xanh dương), Private Event (vàng), Private Competition (cam), Maintenance (xám), Alobo (tím)\n8. Vào tab **Schedule** để cấu hình lịch hàng tuần định kỳ\n9. Vào tab **Pricing** để đặt quy tắc giá theo ngày/giờ\n\nCác buổi học huấn luyện cũng hiển thị trên lưới này — quản lý từ trang **Coaching**",
...
const CACHED_TH: Record<string, string> = {
  ...
  bookings: "วิธีจัดการการจองสนาม:\n1. ไปที่ **Bookings** ในเมนูซ้าย (ส่วน CourtPass - Booking)\n2. เลือกสถานที่ — ตาราง Day Planner แสดงสนามเป็นคอลัมน์และช่วงเวลาเป็นแถว\n3. ตารางแสดงค่าเริ่มต้น **1 ชั่วโมง**; สลับเป็น **30 นาที** เพื่อดูรายละเอียดมากขึ้น\n4. คลิกช่องว่างเพื่อสร้างการจอง (ผู้เล่นสามารถจองกลุ่มหลายสนามได้)\n5. คลิกการจองที่มีอยู่เพื่อแก้ไขหรือยกเลิก\n6. ใช้ลูกศรเพื่อเปลี่ยนวัน\n7. ช่องที่ถูกบล็อกแสดงเป็นบล็อกสี: Open Play (เขียว), Competition (น้ำเงิน), Private Event (เหลือง), Private Competition (ส้ม), Maintenance (เทา), Alobo (ม่วง)\n8. ไปที่แท็บ **Schedule** เพื่อตั้งค่าตารางประจำสัปดาห์\n9. ไปที่แท็บ **Pricing** เพื่อตั้งกฎราคาตามวัน/เวลา\n\nคาบสอนโค้ชก็ปรากฏบนตารางเดียวกัน — จัดการจากหน้า **Coaching**",
