...
export default function AnalyticsPage() {
  ...
  const MATCH_LABELS: Record<string, string> = {
    too_easy: t("analytics.tooEasy"),
    perfect: t("analytics.balanced"),
    too_hard: t("analytics.tooHard"),
  };

  const RETURN_LABELS: Record<string, string> = {
    yes: t("analytics.wouldReturn"),
    maybe: t("analytics.maybe"),
    no: t("analytics.wouldNotReturn"),
  };
  ...
  if (!data) return <p className="text-neutral-500">{t("analytics.loading")}</p>;

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-xl font-bold md:text-2xl">{t("analytics.title")}</h2>
        <select
          value={selectedVenue}
      ...
      <div className="grid gap-3 grid-cols-3 md:gap-4">
        <StatCard icon={Users} label={t("analytics.players")} value={data.overview.totalPlayers} color="text-green-500" />
        <StatCard icon={Trophy} label={t("analytics.gamesPlayed")} value={data.overview.totalGames} color="text-blue-500" />
        <StatCard icon={BarChart3} label={t("analytics.sessions")} value={data.overview.totalSessions} color="text-purple-500" />
      </div>

      <div>
        <h3 className="mb-3 text-lg font-semibold text-neutral-300">{t("analytics.venueBreakdown")}</h3>
          ...
          {data.venues.map((v) => (
            <div key={v.id} className="rounded-xl border border-neutral-800 bg-neutral-900 p-3 md:p-4">
              ...
              <div className="flex gap-3 text-xs md:text-sm text-neutral-400">
                <span>{v.courts} {t("analytics.courts")}</span>
                <span>{v.sessions} {t("analytics.sessions_label")}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        <h3 className="mb-3 text-lg font-semibold text-neutral-300">{t("analytics.sessionHistory")}</h3>
        ...
        <div className="hidden md:block rounded-xl border border-neutral-800">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-neutral-800 text-neutral-400">
              <tr>
                <th className="px-4 py-3">{t("analytics.venue")}</th>
                <th className="px-4 py-3">{t("analytics.date")}</th>
                <th className="px-4 py-3">{t("analytics.status")}</th>
                <th className="px-4 py-3 text-right">{t("analytics.playersCol")}</th>
                <th className="px-4 py-3 text-right">{t("analytics.games")}</th>
                <th className="px-4 py-3 text-right">{t("analytics.playTime")}</th>
                <th className="px-4 py-3 text-right">{t("analytics.waitTime")}</th>
                <th className="px-4 py-3 text-right">{t("analytics.waitPlay")}</th>
              </tr>
            </thead>
            <tbody>
          ...
          {data.recentSessions.map((s) => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-neutral-400">
                ...
                <span>{s.players} {t("analytics.players_mobile")}</span>
                <span>{s.games} {t("analytics.games_mobile")}</span>
                <span>{fmtMin(s.totalPlayMinutes)} {t("analytics.play")}</span>
                <span>{fmtMin(s.totalWaitMinutes)} {t("analytics.wait")}</span>
                <RatioBadge ratio={s.waitPlayRatio} />
              </div>
            </button>
          ))}
          {data.recentSessions.length === 0 && (
            <p className="py-6 text-center text-sm text-neutral-500">{t("analytics.noSessionsYet")}</p>
          )}
        </div>
        <p className="mt-2 text-xs text-neutral-500">{t("analytics.clickSession")}</p>
      </div>
          ...
          >
            <div className="flex items-start justify-between gap-3 border-b border-neutral-800 px-4 py-3 md:px-5">
              <div className="min-w-0">
                <h4 className="text-base font-semibold text-white md:text-lg">{t("analytics.sessionDetail")}</h4>
                {sessionDetail && (
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 md:px-5 md:py-5">
              {detailLoading && (
                <p className="text-sm text-neutral-500">{t("analytics.loadingSession")}</p>
              )}
              {detailError && (
                ...
                <div className="space-y-6">
                  ...
                  <section>
                    <h5 className="mb-2 flex items-center gap-2 text-sm font-semibold text-neutral-300">
                      ...
                      {t("analytics.playerSurveys")}
                    </h5>
                    {sessionDetail.surveySummary.responseCount === 0 ? (
                      <p className="text-sm text-neutral-500">{t("analytics.noSurveys")}</p>
                    ) : (
                      <div className="space-y-3">
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                          <div className="rounded-lg border border-neutral-800 bg-neutral-950/50 p-3">
                            <p className="text-[11px] text-neutral-500">{t("analytics.avgExperience")}</p>
                          ...
                          <div className="rounded-lg border border-neutral-800 bg-neutral-950/50 p-3">
                            <p className="text-[11px] text-neutral-500">{t("analytics.responses")}</p>
                            <p className="text-lg font-semibold tabular-nums">
                          ...
                          <div className="rounded-lg border border-neutral-800 bg-neutral-950/50 p-3 col-span-2">
                            <p className="text-[11px] text-neutral-500 mb-1">{t("analytics.matchLevel")}</p>
                            <div className="flex flex-wrap gap-1.5 text-[11px]">
                              <span className="rounded bg-amber-500/15 px-1.5 py-0.5 text-amber-400">
                                {t("analytics.tooEasy")} {sessionDetail.surveySummary.matchQualityCounts.too_easy}
                              </span>
                              <span className="rounded bg-green-500/15 px-1.5 py-0.5 text-green-400">
                                {t("analytics.balanced")} {sessionDetail.surveySummary.matchQualityCounts.perfect}
                              </span>
                              <span className="rounded bg-rose-500/15 px-1.5 py-0.5 text-rose-400">
                                {t("analytics.tooHard")} {sessionDetail.surveySummary.matchQualityCounts.too_hard}
                              </span>
                            </div>
                          </div>
                        </div>
                  ...
                  <section>
                    <h5 className="mb-2 flex items-center gap-2 text-sm font-semibold text-neutral-300">
                      ...
                      {t("analytics.games_section")} ({sessionDetail.games.length})
                    </h5>
                    {sessionDetail.games.length === 0 ? (
                      <p className="text-sm text-neutral-500">{t("analytics.noGames")}</p>
                    ) : (
                      <div className="overflow-x-auto rounded-lg border border-neutral-800">
                        <table className="w-full min-w-[520px] text-left text-xs">
                          <thead className="border-b border-neutral-800 bg-neutral-950/50 text-neutral-500">
                            <tr>
                              <th className="px-3 py-2 font-medium">{t("analytics.court")}</th>
                              <th className="px-3 py-2 font-medium">{t("analytics.type")}</th>
                              <th className="px-3 py-2 font-medium">{t("analytics.duration")}</th>
                              <th className="px-3 py-2 font-medium">{t("analytics.playersSurveyed")}</th>
                            </tr>
                          </thead>
                          <tbody>
