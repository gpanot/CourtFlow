...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!entry) return error("Player not found in waiting queue");

    await prisma.queueEntry.update({
      where: { id: entry.id },
      data: { status: "on_break", breakUntil: null, groupId: null },
    });

    if (entry.group) {
      const remaining = entry.group.queueEntries.filter((e) => e.id !== entry.id && e.status !== "left");
    ...
    console.error("[Staff Break] Error:", e);
